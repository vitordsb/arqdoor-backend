...
const createLocationUserValidator = async (req, res, next) => {
  try {
    ...
    const schema = Joi.object({
      ...
      state: Joi.string()
        ...
        .pattern(/^[A-Z]+/)
        .required(),
      city: Joi.string().min(3).max(58).required(),
    ...
    return res.status(500).json({
      ...
      error: {
        details: [
          {
            middleware: "createLocationUserValidator",
            message: "Erro interno",
          },
        ],
      },
      message: "Erro no createLocationUserValidator",
      success: false,
    });
  }
};

module.exports = createLocationUserValidator;
